...
The AI narrator (`ai/narrator.py`) receives the fully-computed `ScoreResult` (and the `WhatChanged` diff, if one exists) as structured JSON and is explicitly instructed to write a bull case, a bear case, a confidence level, catalysts to watch, risk factors, and what would change the reader's mind, not to recompute or second-guess the scores, and not to issue a Buy, Hold, or Sell recommendation. Every catalyst and risk factor is tagged with a `claim_type` (Fact, Inference, or Opinion) and a `based_on` field naming the specific category or metric it's traceable to, so the reader can judge how much weight to give each claim rather than treating all AI output as equally certain. If a threshold above seems wrong for a given ticker, the fix is in this table, not in the AI layer.

If a prior thesis exists for the ticker (see `thesis_history.py`), its `what_would_change_my_mind` conditions are passed into the same call as `previous_invalidation_conditions`, and the model classifies each one Triggered / Not triggered / Unclear against today's data in the `invalidation_check` field -- this piggybacks on the existing single call rather than adding a second one, and it only runs when you manually research that ticker again, not on a schedule.

The same call also receives the prior thesis's catalysts and risk factors as `previous_claims` and judges each testable one Held up / Did not hold up / Too early to tell in `claim_accuracy_check`, strictly against the fundamental metric it was grounded in (its `based_on` field) -- never against the stock price, since that would function as a covert Buy/Sell signal. `accuracy.py` aggregates these across every recorded thesis into a running "Track record" on the report page. No new persistence store or AI call; it's a pure aggregation over `thesis_history.py`'s existing data.
