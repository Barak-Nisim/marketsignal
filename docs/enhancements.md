...
11. **[Shipped]** ~~Side-by-side comparison view for two tickers.~~ Shipped via `comparison.py` (pure, no network/AI: zips two ScoreResults' five categories and says which score is numerically higher per category and overall -- never a buy/sell call) plus `marketsignal compare TICKER_A TICKER_B` on the CLI and a `/compare` form + result page on the web. Scoped to overall + category-level scores, not full metric-level detail; each ticker's own report is one click away for that.
